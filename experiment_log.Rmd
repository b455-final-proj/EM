...
ans2 = EM(data=x, n.classes=3, prior.choice='equal.priors.random.params', max.iter=1000)

y = generate.mog(n.data=1000, mu=as.numeric(ans2$mu), sigma=ans2$sigma, prior=ans2$prior)
hist(y, breaks=100)
```

We were able to implement a function which generates multivariate MOGs. However, we did not bother to implement the multivariate EM method because we already run into issues in the univariate setting.

```{r}
set.seed(0)
x = generate.mog(n.data=1000, mu=matrix(c(1:6), nrow=3),
                 sigma=replicate(3, matrix(c(2,1,1,2), nrow=2)), 
                 prior=c(.6,.2,.2))
hist(x, breaks=100)
